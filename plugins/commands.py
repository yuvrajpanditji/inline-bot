...
async def start(bot, message):
    if message.from_user.id in info.BANNED_USERS:
        await message.reply_text("Sorry, You are banned to use me ☹️ Please Contact  Pandit Ji! 😊")
        return
    ...
    if len(message.command) > 1 and message.command[1] == 'subscribe':
        ...
        buttons = [
            [
                InlineKeyboardButton('Updates Channel 🗣', url='https://t.me/pandityuvrajsharma'),
                InlineKeyboardButton('Go Inline 🎭', switch_inline_query=''),
            ],
            [
                InlineKeyboardButton('Search E-books  🔎', switch_inline_query_current_chat=''),
            ],
        ]
...
async def total(bot, message):
    ...
    msg = await message.reply("Processing...⏳\@pandityuvrajsharma BotPandit", quote=True)
    try:
        total = await Media.count_documents()
        await msg.edit(f'📁 Total files saved in PJ Search Bot Database: {total}')
...
async def delete(bot, message):
    ...
    if result.deleted_count:
        await msg.edit('File is successfully deleted from database\@pandityuvrajsharma BotPandit')
    else:
        await msg.edit('File not found in database\@pandityuvrajsharma BotPandit')
